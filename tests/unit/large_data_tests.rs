...
fn test_array_id_key_performance() {
    ...
    println!("Positional comparison: {} diffs in {:?}", positional_results.len(), positional_duration);
    println!("ID-based comparison: {} diffs in {:?}", id_results.len(), id_duration);
    ...
    println!("Optimized ID-based: {} diffs in {:?}", optimized_results.len(), optimized_duration);
}
...
fn test_regex_filtering_performance() {
    ...
    for i in 0..1000 {
        ...
        data1.insert(format!("_internal_{}", i), json!(format!("internal_value_{}", i)));
        data1.insert(format!("timestamp_{}", i), json!(format!("2023-01-01T{:02}:00:00Z", i % 24)));
        
        // Add keys that should be compared
        data1.insert(format!("config_{}", i), json!({"value": i}));
        
        // Same for data2, but with changes
        data2.insert(format!("_internal_{}", i), json!(format!("different_internal_{}", i))); // Should be ignored
        data2.insert(format!("timestamp_{}", i), json!(format!("2023-01-02T{:02}:00:00Z", i % 24))); // Should be ignored
        data2.insert(format!("config_{}", i), json!({"value": if i % 10 == 0 { i + 1 } else { i }})); // Should be compared
    }
    ...
    println!("All differences: {} in {:?}", all_results.len(), all_duration);
    println!("Filtered differences: {} in {:?}", filtered_results.len(), filtered_duration);
    ...
    println!("Optimized filtered: {} in {:?}", optimized_results.len(), optimized_duration);
}