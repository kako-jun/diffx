...
"#).unwrap();
    ...
    let output = Command::new("cargo")
        ...
        .arg(&manifest2)
        .arg("--ignore-keys-regex")
        .arg("^metadata\\.(creationTimestamp|resourceVersion)$")
        .arg("--format")
        .arg("yaml")
        .output()
        .expect("Failed to execute command");
