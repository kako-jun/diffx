...
fn test_path_filtering_application() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.arg("../tests/fixtures/config_dev.json")
       ...
       .arg("--path").arg("application");
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("~ application.debug: true -> false"))
        .stdout(predicate::str::contains("~ application.environment: \"development\" -> \"production\""))
...
fn test_path_filtering_database() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.arg("../tests/fixtures/config_dev.json")
       .arg("../tests/fixtures/config_prod.json")
       .arg("--path").arg("database");
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("~ database.host: \"localhost\" -> \"prod-db.example.com\""))
        .stdout(predicate::str::contains("~ database.name: \"myapp_dev\" -> \"myapp_prod\""))
...
fn test_complex_regex_security_fields() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.arg("../tests/fixtures/security_config.json")
       ...
       .arg("--ignore-keys-regex").arg("^(password|secret_.*|credentials|connection_string)$");
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("~ application.version: \"1.0.0\" -> \"1.1.0\""))
        .stdout(predicate::str::contains("~ security.host: \"localhost\" -> \"prod-server.example.com\""))
...
fn test_complex_regex_build_fields() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.arg("../tests/fixtures/security_config.json")
       .arg("../tests/fixtures/security_config_new.json")
       .arg("--ignore-keys-regex").arg("^(timestamp|build_.*|deploy_.*)$");
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("~ application.version: \"1.0.0\" -> \"1.1.0\""))
        .stdout(predicate::str::contains("~ monitoring.metrics.cpu: 45.2 -> 52.1"))
        .stdout(predicate::str::contains("~ monitoring.metrics.memory: 78.9 -> 82.3"))
...
fn test_complex_regex_multiple_groups() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.arg("../tests/fixtures/security_config.json")
       .arg("../tests/fixtures/security_config_new.json")
       .arg("--ignore-keys-regex").arg("^(password|secret_.*|timestamp|build_.*)$");
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("~ application.version: \"1.0.0\" -> \"1.1.0\""))
        .stdout(predicate::str::contains("~ security.host: \"localhost\" -> \"prod-server.example.com\""))
...
fn test_combined_path_and_regex() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.arg("../tests/fixtures/security_config.json")
       .arg("../tests/fixtures/security_config_new.json")
       .arg("--path").arg("monitoring")
       .arg("--ignore-keys-regex").arg("^(timestamp|build_.*)$");
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("~ monitoring.metrics.cpu: 45.2 -> 52.1"))
        .stdout(predicate::str::contains("~ monitoring.metrics.memory: 78.9 -> 82.3"))
...
fn test_combined_array_id_and_epsilon() -> Result<(), Box<dyn std::error::Error>> {
  ...
  "records": [
    ...
    cmd.arg("../tests/fixtures/array_epsilon1.json")
       ...
       .arg("--epsilon").arg("0.0001");
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("~ records[id=2].name: \"item2\" -> \"item2_updated\""))
...
fn test_format_specification_with_stdin() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cmd = diffx_cmd();
    let mut child = cmd.arg("-").arg("../tests/fixtures/file2.ini").arg("--format").arg("ini")
        .stdin(std::process::Stdio::piped())
        .stdout(std::process::Stdio::piped())
        .stderr(std::process::Stdio::piped())
        .spawn()?;
...
"#)?;
    ...
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(predicate::str::contains("~ section1.key2: \"value2\" -> \"new_value2\"").eval(&stdout));
...
fn test_cicd_configuration_drift() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.arg("../tests/fixtures/security_config.json")
       .arg("../tests/fixtures/security_config_new.json")
       .arg("--ignore-keys-regex").arg("^(timestamp|build_.*|deploy_.*|password|secret_.*)$")
       .arg("--output").arg("yaml");
    cmd.assert()
        .success()
...
fn test_environment_config_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.arg("../tests/fixtures/config_dev.json")
       .arg("../tests/fixtures/config_prod.json")
       .arg("--path").arg("application")
       .arg("--ignore-keys-regex").arg("^(host|port|password|.*_secret)$");
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("~ application.debug: true -> false"))
        .stdout(predicate::str::contains("~ application.environment: \"development\" -> \"production\""))
