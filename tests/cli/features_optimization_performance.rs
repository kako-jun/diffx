...
fn test_deep_nesting_optimization() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(stdout.contains("deep_value") && (stdout.contains("original") || stdout.contains("modified")));
...
fn test_streaming_large_files() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..100000 {
        ...
        csv2.push_str(&format!(
            ...
            i, i, i, 
            if i % 1000 == 0 { "Management" } else { "Engineering" },  // Some dept changes
    ...
    let start = Instant::now();
    let mut cmd = diffx_cmd();
    cmd.arg(&stream_file1)
        .arg(&stream_file2)
        .arg("--brief");  // Use brief mode for large files
...
fn test_concurrent_optimization() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(final_count, 3, "Concurrent optimization failed: only {} of 3 succeeded", final_count);
...
fn test_performance_regression() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(ratio < 100.0, "Performance scaling poor: {}x slower for medium files", ratio);
...
fn test_optimization_effectiveness() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..10000 {
        ...
        xml2.push_str(&format!(
            "  <item id=\"{}\">\n    <name>Item {}</name>\n    <value>{}</value>\n  </item>\n",
            i, i, i * 10 + (if i == 5000 { 1 } else { 0 })  // One small change
        ));
    }
