...
if [ -f "tests/test_integration.py" ] || [ -f "test_integration.py" ]; then
    echo "🧪 Running tests..."
    python -m pytest tests/ || python -m pytest . || echo "⚠️  Tests failed or not configured properly"
else
...
if command -v diffx-python &> /dev/null; then
    diffx-python --help || echo "⚠️  Package installed but CLI not working properly"
else
    echo "⚠️  CLI command not available after installation"
fi
