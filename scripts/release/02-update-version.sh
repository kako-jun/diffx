...
update_npm_version() {
    local new_version=$1
    local package_json="$PROJECT_ROOT/diffx-npm/package.json"
    
    if [ -f "$package_json" ]; then
        echo "Updating version in diffx-npm/package.json"
        cd "$PROJECT_ROOT/diffx-npm"
        npm version "$new_version" --no-git-tag-version
        cd "$PROJECT_ROOT"
    else
        echo "WARNING: diffx-npm/package.json not found"
    fi
}
...
main() {
    ...
    update_version_in_file "$PROJECT_ROOT/diffx-core/Cargo.toml" "$current_version" "$new_version"
    update_version_in_file "$PROJECT_ROOT/diffx-cli/Cargo.toml" "$current_version" "$new_version"
    
    # Update Python package
    update_version_in_file "$PROJECT_ROOT/diffx-python/pyproject.toml" "$current_version" "$new_version"
    update_version_in_file "$PROJECT_ROOT/diffx-python/Cargo.toml" "$current_version" "$new_version"
