...
## 🔨 Phase 2.5: リファクタリング（進行中）

### diffx-core のリファクタリング
- [x] types.rs を抽出（DiffResult, DiffOptions など）
- [x] parser/ モジュールを作成（6フォーマット + format.rs）
- [ ] lib.rs を更新（モジュールインポート、既存コード削除）
- [ ] diff/ モジュールを作成（差分検出ロジック）
- [ ] io/ モジュールを作成（ファイル・ディレクトリ操作）
- [ ] コンパイル確認
- [ ] 基本動作確認（6フォーマット）
