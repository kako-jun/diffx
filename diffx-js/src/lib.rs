...
use diffx_core::{
    diff as core_diff, parse_json as core_parse_json, parse_csv as core_parse_csv, 
    parse_yaml as core_parse_yaml, parse_toml as core_parse_toml, parse_ini as core_parse_ini, 
    parse_xml as core_parse_xml, DiffOptions, DiffxSpecificOptions, OutputFormat, 
    format_output as core_format_output, DiffResult
};
use regex::Regex;
...
pub fn format_output(results: Vec<JsDiffResult>, format: String) -> Result<String> {
    ...
    let rust_results = results.into_iter()
...
fn convert_js_diff_result(js_result: JsDiffResult) -> Result<DiffResult> {
    match js_result.diff_type.as_str() {
        "Added" => {
            let value = js_result.new_value
                .ok_or_else(|| Error::new(Status::InvalidArg, "Added result must have new_value"))?;
        ...
        "Modified" => {
            let old_value = js_result.old_value
                .ok_or_else(|| Error::new(Status::InvalidArg, "Modified result must have old_value"))?;
            let new_value = js_result.new_value
                .ok_or_else(|| Error::new(Status::InvalidArg, "Modified result must have new_value"))?;
            Ok(DiffResult::Modified(js_result.path, old_value, new_value))
        },
        "TypeChanged" => {
            let old_value = js_result.old_value
                .ok_or_else(|| Error::new(Status::InvalidArg, "TypeChanged result must have old_value"))?;
            let new_value = js_result.new_value
                .ok_or_else(|| Error::new(Status::InvalidArg, "TypeChanged result must have new_value"))?;
            Ok(DiffResult::TypeChanged(js_result.path, old_value, new_value))
        },
        _ => Err(Error::new(Status::InvalidArg, format!("Invalid diff result type: {}", js_result.diff_type))),
    }
}