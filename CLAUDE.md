...
## 🚨 重要な未実装機能・バグ (2025-01-10)

### 1. Unified出力形式でのパスフィルタリング未対応
- **問題**: `--output unified` 使用時に `--path` フィルタが無視される
- **現象**: フィルタ指定してもファイル全体の差分が表示される
- **影響**: `--path services` と `--ignore-keys-regex` を組み合わせても、unified出力では全フィールドが表示される
- **修正箇所**: `print_unified_output` 関数でフィルタリングされた値を使用するべき

### 2. 配列要素の追加/削除時のignore_keys_regex
- **問題**: 配列要素が完全に追加/削除された場合、`--ignore-keys-regex` が適用されない
- **現象**: 要素の修正時は無視されるが、追加/削除時は全フィールドが表示される
- **設計判断**: これは仕様として正しい可能性もある（削除/追加は完全に表示すべき）

## 🔄 最新の重要変更 (2025-07-10)

### 設定ファイル・環境変数サポート削除
- **削除理由**: 兄弟アプリ（diffai, lawkit）との一貫性確保、UNIX哲学の遵守
- **削除された機能**:
  - `~/.config/diffx/config.toml` 設定ファイル読み込み
  - 環境変数サポート（`DIFFX_OUTPUT`, `DIFFX_IGNORE_KEYS_REGEX`, `DIFFX_EPSILON`）
  - 関連するテストケース（13個のテスト削除）
- **影響**: コア機能は全て維持、設定方法がCLI引数のみに簡素化
- **テスト状況**: 全73テスト成功（環境変数・設定ファイル関連テスト削除後）

## 🎉 最新完了作業 (2025-01-10)

### 🐛 重要バグ修正・テスト改善
- **終了コード問題**: 差分あり時に終了コード1、差分なし時に終了コード0、エラー時に終了コード2に修正
- **環境変数サポート**: DIFFX_OUTPUT, DIFFX_IGNORE_KEYS_REGEX, DIFFX_EPSILON の完全実装（※後に削除）
- **CLI出力修正**: 差分なし時の出力を空に変更（diff標準準拠）
- **エラーハンドリング強化**: 完全なエラーチェーン表示（{:#}フォーマット使用）

### 🧪 テスト状況大幅改善
- **ドキュメントテスト**: 29/29 通過（100%成功率）
- **主要修正**: 一時ファイルへの --format json 指定追加
- **環境変数テスト**: 全て通過
- **終了コードテスト**: 全て通過
...
- **成功率**: 63/91テスト通過（69%）、大幅改善済み

### 🔧 実装修正完了（2025-07-09）
- **終了コード修正**: 差分あり→終了コード1、差分なし→終了コード0
- **環境変数サポート**: DIFFX_OUTPUT, DIFFX_IGNORE_KEYS_REGEX, DIFFX_EPSILON
- **出力形式修正**: 差分なし時は空の出力（diff標準準拠）
- **テスト修正**: 63/91テスト通過、期待値とのアライメント完了
- **設定ファイル修正**: format設定でのEnum値正規化（"json"→"Json"）
