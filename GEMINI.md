...
- **❌ 差分表示のコンテキスト**: CLI表示で、変更された値の周辺の構造をより分かりやすく表示する。
...
- **❌ CI/CDとの統合例**: GitHub ActionsやGitLab CI/CDでの利用例を `docs/` に追加する。
...
2. **全フォーマット対応** - JSON/YAML/TOML完全対応
...
1. **出力形式** - TOML出力のみ未対応（技術的制約）

### ❌ 未実装 (Not Implemented)
1. **差分表示コンテキスト** - 変更周辺の構造をより分かりやすく表示
2. **他言語ラッパー** - npm/pipパッケージ
3. **CI/CD統合例** - GitHub Actions等の利用例
4. **詳細ドキュメント** - 使用例/FAQ/貢献ガイドライン
