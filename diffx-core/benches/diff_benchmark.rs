...
fn benchmark_different_sizes(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        let data_size = serde_json::to_string(&v1).unwrap().len() + 
                       serde_json::to_string(&v2).unwrap().len();
        group.throughput(Throughput::Bytes(data_size as u64));
        
        group.bench_with_input(
            BenchmarkId::new("objects", size),
            &size,
            |b, _| {
...
fn benchmark_large_file_processing(c: &mut Criterion) {
    ...
    temp_file2.write_all(serde_json::to_string_pretty(&large_data.1).unwrap().as_bytes()).unwrap();
